...
$no = 1;
$allArtikel = mysqli_query($mysqli, "SELECT artikel.*,
                            kategori.nama_kategori,
                            users.nama_operator
                            FROM artikel
                            INNER JOIN kategori ON artikel.id_kategori = kategori.id
...
$new_artikel = mysqli_query($mysqli, "SELECT artikel.*,
                            kategori.nama_kategori,
                            users.nama_operator
                            FROM tb_artikel
                            INNER JOIN tb_kategori ON artikel.id_kategori = kategori.id
                            INNER JOIN tb_users ON artikel.user_id = users.id
                            LIMIT $halaman_awal, $batas
                           ");

$artikel = mysqli_query($mysqli, "SELECT artikel.*,
                            kategori.nama_kategori,
                            users.nama_operator
                            FROM artikel
                            INNER JOIN kategori ON artikel.id_kategori = kategori.id
...
$agenda = mysqli_query($mysqli, "SELECT agenda.*,
                          kategori.nama_kategori,
                          users.nama_operator
                          FROM agenda
                          INNER JOIN kategori ON agenda.id_kategori = kategori.id
                          INNER JOIN users ON agenda.user_id = users.id
                          ORDER BY id DESC
                          limit 4
                          ");


$about = mysqli_query($mysqli, "SELECT * from tentang");
$kategori = mysqli_query($mysqli, "SELECT * from kategori");
$menu = mysqli_query($mysqli, "SELECT * from menu");
